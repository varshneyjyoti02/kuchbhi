...
isrunning() {
  ...
  if [ -f "$CATALINA_PID" ]; then
	  ...
	  LINES=`$LINES $PID 2> /dev/null`
      PIDRET=$?
      # Add grep for supported platforms
      if [ $RUNGREP = "true" ] ; then
          # This check is to make sure that the pid found matches 
          # the correct tc Runtime. On an system crash, it is possible
          # the pid could match another process and fail on startup.
          MATCH=`echo $LINES | grep "\-Dcatalina.base=$CATALINA_BASE"`
          PIDRET=$?
      fi
      if [ $PIDRET -eq 0 ]; then
          export PID
